...
namespace GameBoySharp.Emu.GamePak
{
    public class MBC3 : GameRom
    {
        ...
        public MBC3(byte[] rom) : base(rom) { }

        public override byte ReadERAM(ushort addr)
        {
            if (ERAM_ENABLED)
            {
                return RAM_BANK switch
                {
                    int _ when RAM_BANK >= 0x00 && RAM_BANK <= 0x03 => ERAM[(ERAM_OFFSET * RAM_BANK) + (addr & 0x1FFF)],
                    0x08 => RTC_S,
                    0x09 => RTC_M,
                    0x0A => RTC_H,
                    0x0B => RTC_DL,
        ...
        public override void WriteROM(ushort addr, byte value)
        {
            switch (addr)
            {
                ...
                case ushort _ when addr >= 0x4000 && addr <= 0x5FFF:
                    switch (value)
                    {
                        case byte _ when value >= 0x00 && value <= 0x03:
                        case byte _ when value >= 0x08 && value <= 0xC0:
                            RAM_BANK = value;
                            break;
                    }
                    break;
                case ushort _ when addr >= 0x6000 && addr <= 0x7FFF:
